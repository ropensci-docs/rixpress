...
[![Video
Thumbnail](https://raw.githubusercontent.com/ropensci/rixpress/refs/heads/main/video_thumbnail.png)](https://www.youtube.com/watch?v=a1eNG9TFZ_o)
...
the user experience of the [targets](https://docs.ropensci.org/targets/)
package and builds on the [rix](https://docs.ropensci.org/rix/) package
to obtain fully reproducible development/runtime environments.
...
- Reuse hermetic environments (pinned system + language dependencies)
  defined via [rix](https://docs.ropensci.org/rix/) and a `default.nix`.
...
The [rix](https://docs.ropensci.org/rix/) package brings this power into
the R ecosystem. It makes it easy to generate declarative, date-pinned
Nix expressions that work consistently across systems. This expressions
can then be used to build reproducible development environments that
include programming languages, packages and other tools.
[rixpress](https://docs.ropensci.org/rixpress) addresses the next part,
the execution graph. With a concise R API, it lets users describe
...
If you are interested in a Python-first port of
[rixpress](https://docs.ropensci.org/rixpress), check out
...
- **Sub-pipelines**: Organize large projects with
  [`rxp_pipeline()`](https://docs.ropensci.org/rixpress/reference/rxp_pipeline.md)
  for logical grouping and colour-coded visualization.
- DAG visualization
  (e.g. [`rxp_ggdag()`](https://docs.ropensci.org/rixpress/reference/rxp_ggdag.md),
  [`rxp_visnetwork()`](https://docs.ropensci.org/rixpress/reference/rxp_visnetwork.md))
  with pipeline-aware colouring.
...
**sub-pipelines** using
[`rxp_pipeline()`](https://docs.ropensci.org/rixpress/reference/rxp_pipeline.md).
Each sub-pipeline can have its own colour for visual distinction:
...
When visualized with
[`rxp_ggdag()`](https://docs.ropensci.org/rixpress/reference/rxp_ggdag.md),
nodes show: - **Fill colour**: Derivation type (R = blue, Python =
yellow, etc.) - **Border colour**: Pipeline group (the colour you
specify)

This dual encoding lets you see both *what* each step does and *which
logical group* it belongs to. See
[`vignette("sub-pipelines")`](https://docs.ropensci.org/rixpress/articles/sub-pipelines.md)
for details.
...
generates a `pipeline.nix` file, which contains the build instructions
for all derivations and final outputs expressed as Nix code. You can
define derivations that run Python or Julia code, and objects can be
exchanged between R and Python by using
...
or by serializing to a common format such as JSON. By default, calling
[`rxp_populate()`](https://docs.ropensci.org/rixpress/reference/rxp_populate.md)
also builds the pipeline, but it’s possible to only generate the
`pipeline.nix` file and build the pipeline later using:
...
[rixpress](https://docs.ropensci.org/rixpress) provides several helper
functions:
...
[reticulate](https://rstudio.github.io/reticulate/) is available in the
environment. For complex outputs such as documents (for example the
Quarto document `page` above), `rxp_read("page")` returns the output
file path; you can then open it with
[`browseURL()`](https://rdrr.io/r/utils/browseURL.html) or copy it into
your working directory with
[`rxp_copy()`](https://docs.ropensci.org/rixpress/reference/rxp_copy.md).

[rixpress](https://docs.ropensci.org/rixpress) is flexible; please
consult the examples repository for many different patterns and complete
demos:
...
While [rixpress](https://docs.ropensci.org/rixpress) is a regular R
package, there is little point in using it without having Nix installed.
If you are not familiar with Nix, I recommend you first start by
checking out [rix](https://docs.ropensci.org/rix/), which will teach you
how to use Nix to set up reproducible development environments for your
R projects!
...
to learn more about what [rixpress](https://docs.ropensci.org/rixpress)
will and will not support.

## Acknowledgements

- Inspired by the design and ergonomics of
  [targets](https://docs.ropensci.org/targets/).
- Built atop the reproducibility foundations provided by
  [rix](https://docs.ropensci.org/rix/) and the ‘Nix’ ecosystem.
